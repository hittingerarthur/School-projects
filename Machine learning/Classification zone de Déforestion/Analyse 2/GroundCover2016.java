...
// ===============================
// DRC LAND COVER CLASSIFICATION (2016)
// ===============================
...
// Load and process Sentinel-2 for 2016
var s2_2016 = ee.ImageCollection("COPERNICUS/S2")
...
// Calculate NDVI from Sentinel-2 (2016)
var ndvi_2016 = s2_2016.normalizedDifference(["B8", "B4"]).rename("NDVI");

// Load Sentinel-1 VV polarization for 2016
...
var input_features_2016 = s2_2016
  .addBands(ndvi_2016)
  .addBands(s1_2016)
  .addBands(elevation)
  .addBands(slope);
...
var training_2016 = input_features_2016.sampleRegions({
...
var classifier_2016 = ee.Classifier.smileRandomForest(100).train({
  features: training_2016,
  classProperty: "class",
  inputProperties: bands
});

// Classify the 2016 image
var classified_2016 = input_features_2016.classify(classifier_2016);

// ========== VISUALIZATION ==========

// NDVI visualization
var ndviVis = {min: 0, max: 1, palette: ["yellow", "green"]};
Map.addLayer(ndvi_2016, ndviVis, "NDVI (2016)");

// Radar visualization
var radarVis = {min: -20, max: 0, palette: ["white", "black"]};
Map.addLayer(s1_2016, radarVis, "Radar VV (2016)");
...
Map.addLayer(classified_2016, classVis, "Land Cover Classification (2016)");
...
var withRandom = training_2016.randomColumn();
var split = 0.7;
...
Export.image.toDrive({
  ...
  folder: 'GEE_exports',
  fileNamePrefix: 'classified_2016',
  region: drc_roi,
  scale: 30,
  crs: 'EPSG:4326',
  maxPixels: 1e13
...
var points = classified_2016.sample({
  region: drc_roi,
  scale: 30,
...
Export.table.toDrive({
  collection: points,
  description: 'classified_points_2016',
  fileFormat: 'CSV'
});
