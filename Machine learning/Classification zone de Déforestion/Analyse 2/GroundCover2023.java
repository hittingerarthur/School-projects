...
// ===============================
// DRC LAND COVER CLASSIFICATION (2023)
// ===============================
...
// Load and process Sentinel-2 for 2023
...
// Calculate NDVI from Sentinel-2 (2023)
var ndvi_2023 = s2_2023.normalizedDifference(["B8", "B4"]).rename("NDVI");

// Load Sentinel-1 VV polarization for 2023
var s1_2023 = ee.ImageCollection("COPERNICUS/S1_GRD")
  .filterBounds(drc_roi)
  .filterDate("2023-01-01", "2023-12-31")
...
var input_features_2023 = s2_2023
  .addBands(ndvi_2023)
  .addBands(s1_2023)
  .addBands(elevation)
  .addBands(slope);
...
var training_2023 = input_features_2023.sampleRegions({
...
var classifier_2023 = ee.Classifier.smileRandomForest(100).train({
  features: training_2023,
  classProperty: "class",
  inputProperties: bands
});

// Classify the 2023 image
var classified_2023 = input_features_2023.classify(classifier_2023);

// ========== VISUALIZATION ==========

// NDVI visualization
var ndviVis = {min: 0, max: 1, palette: ["yellow", "green"]};
Map.addLayer(ndvi_2023, ndviVis, "NDVI (2023)");

// Radar visualization
var radarVis = {min: -20, max: 0, palette: ["white", "black"]};
Map.addLayer(s1_2023, radarVis, "Radar VV (2023)");
...
Map.addLayer(classified_2023, classVis, "Land Cover Classification (2023)");
...
var withRandom = training_2023.randomColumn();
var split = 0.7;
...
Export.image.toDrive({
  image: classified_2023,
  description: 'classified_2023',
  folder: 'GEE_exports',
  fileNamePrefix: 'classified_2023',
  region: drc_roi,
  scale: 30,
  crs: 'EPSG:4326',
  maxPixels: 1e13
...
var points = classified_2023.sample({
  region: drc_roi,
  scale: 30,
...
Export.table.toDrive({
  collection: points,
  description: 'classified_points_2023',
  fileFormat: 'CSV'
});