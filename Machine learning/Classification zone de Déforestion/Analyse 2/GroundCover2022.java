...
// ===============================
// DRC LAND COVER CLASSIFICATION (2022)
// ===============================
...
// Load and process Sentinel-2 for 2022
var s2_2022 = ee.ImageCollection("COPERNICUS/S2")
...
// Calculate NDVI from Sentinel-2 (2022)
var ndvi_2022 = s2_2022.normalizedDifference(["B8", "B4"]).rename("NDVI");

// Load Sentinel-1 VV polarization for 2022
var s1_2022 = ee.ImageCollection("COPERNICUS/S1_GRD")
  .filterBounds(drc_roi)
  .filterDate("2022-01-01", "2022-12-31")
...
var input_features_2022 = s2_2022
  .addBands(ndvi_2022)
  .addBands(s1_2022)
  .addBands(elevation)
  .addBands(slope);
...
var training_2022 = input_features_2022.sampleRegions({
...
var classifier_2022 = ee.Classifier.smileRandomForest(100).train({
  features: training_2022,
  classProperty: "class",
  inputProperties: bands
});

// Classify the 2022 image
var classified_2022 = input_features_2022.classify(classifier_2022);

// ========== VISUALIZATION ==========

// NDVI visualization
var ndviVis = {min: 0, max: 1, palette: ["yellow", "green"]};
Map.addLayer(ndvi_2022, ndviVis, "NDVI (2022)");

// Radar visualization
var radarVis = {min: -20, max: 0, palette: ["white", "black"]};
Map.addLayer(s1_2022, radarVis, "Radar VV (2022)");
...
Map.addLayer(classified_2022, classVis, "Land Cover Classification (2022)");
...
var withRandom = training_2022.randomColumn();
var split = 0.7;
...
Export.image.toDrive({
  ...
  folder: 'GEE_exports',
  fileNamePrefix: 'classified_2022',
  region: drc_roi,
  scale: 30,
  crs: 'EPSG:4326',
  maxPixels: 1e13
...
var points = classified_2022.sample({
  region: drc_roi,
  scale: 30,
...
Export.table.toDrive({
  collection: points,
  description: 'classified_points_2022',
  fileFormat: 'CSV'
});