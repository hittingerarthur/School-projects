...
// ===============================
// DRC LAND COVER CLASSIFICATION (2024)
// ===============================
...
// Load and process Sentinel-2 for 2024
...
// Calculate NDVI from Sentinel-2 (2024)
var ndvi_2024 = s2_2024.normalizedDifference(["B8", "B4"]).rename("NDVI");

// Load Sentinel-1 VV polarization for 2024
...
var input_features_2024 = s2_2024
  .addBands(ndvi_2024)
  .addBands(s1_2024)
  .addBands(elevation)
  .addBands(slope);
...
var training_2024 = input_features_2024.sampleRegions({
...
var classifier_2024 = ee.Classifier.smileRandomForest(100).train({
  features: training_2024,
  classProperty: "class",
  inputProperties: bands
});

// Classify the 2024 image
var classified_2024 = input_features_2024.classify(classifier_2024);
...
Map.addLayer(s1_2024, radarVis, "Radar VV (2024)");
...
Map.addLayer(classified_2024, classVis, "Land Cover Classification (2024)");
...
var withRandom = training_2024.randomColumn();
var split = 0.7;
...
Export.image.toDrive({
  ...
  folder: 'GEE_exports',
  fileNamePrefix: 'classified_2024',
  region: drc_roi,
  scale: 30,
  crs: 'EPSG:4326',
  maxPixels: 1e13
...
var points = classified_2024.sample({
  region: drc_roi,
  scale: 30,
...
Export.table.toDrive({
  collection: points,
  description: 'classified_points_2024',
  fileFormat: 'CSV'
});
